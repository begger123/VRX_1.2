...
class ObjectRecognizer():
	def __init__(self):
		...
		self.imagePub = rospy.Publisher("/yolo", Image, queue_size = 1)
                self.detectionPub = rospy.Publisher("/yolo_output", String, queue_size = 1)
		
		print("publisher created")

		self.imageSub = rospy.Subscriber("/wamv/sensors/cameras/front_left_camera/image_raw", Image, self.newColorImage)
                self.taskSub = rospy.Subscriber("/vrx/task/info", Task, self.taskCallback)
	...
	def newColorImage(self, imageMsg):
		    ...
		    self.mainThread()
                    ...
                    #this means that this code will do nothing unless the /vrx/task/info/name is docking
                    doNothing=1
	...
	def mainThread(self):
		...
		self.imagePub.publish(self.bridge.cv2_to_imgmsg(self.detectionImage,"bgr8"))
                temp=String()
                temp.data=label
		self.detectionPub.publish(temp)
		self.rate.sleep()
